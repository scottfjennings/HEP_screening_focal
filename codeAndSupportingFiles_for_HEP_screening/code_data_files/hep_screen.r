...
## 1. The scoring sheet: Either 1 sheet, named 'nests' (for colonies with few nests or species), or a sheet for each species (for large and multi-species colonies) that is equivalent to the handwritten scoring sheet, plus some fields generated by the nest screening algorithm that serve as helpers for the manual screening; the user determines whether 1 or multiple sheets are generated. This sheet has the following fields:
    ...
    # 'days.short.of.fledging' = for nests that didn't clearly fail but were observed innactive earlier than expected, the number of days from the last date it was observed active until the expecteed fledge date; 
    # 'active.last.day' = yes/no (1/0) indicating if nest was still active on the last day it was checked
    # 'max.stage' = maximum stage teh nest was observed in
    # 'num.spp' = number of species assigned to this nest ID- helps find where observer might have been confused about which nest was which, or true species turnovers
    # 'stg4.brood' = maximum confidently observed stage 4 brood size
...
hep_visits <- hep_visits %>% 
  mutate(date = as.Date(date, format = "%d-%b-%y"),
         endtime = as.hms(endtime, tz = "UTC"),
         starttime = as.hms(starttime, tz = "UTC"))
...
totobsdays = hep_visits_sub %>% 
  group_by(species) %>%
  summarize(days = length(starttime))	

totobshours = hep_visits_sub %>% 
  group_by(species) %>%
...
nest_num_stage = hep %>% 
	...
	summarise(num.nests = length(nest)) %>% 
  spread(date, num.nests)
...
num_active_wide <- nests_per_day %>% 
  select(date, species, num.active) %>%  
  spread(date, num.active) 
  ...
  mutate(active.last.day = ifelse(latest.date == spp.max.date, 1, 0), # was the nest still active on the last day of nest checks for this species?
	...
	focal = ifelse(earliest.date <= peak.active.date & # nest was observed active before peak colony size
								...
								max.stage > 0 & # nest actually reached incubation or brooding
									(((earliest.date <= round(earliest.date.chx - (inc.dur - 14), 0)) & min.stage.chx < 4 ) | # nest was first observed during the first 2 weeks of incubation or earlier, based on the first day the nest was observed with chicks
										(earliest.date <= round(latest.date.stage1 - (inc.dur - 14), 0))), 1, 0),# or nest was first observed during the first 2 weeks of incubation or earlier, based on the last day the nest was observed being incubated
	fledged = ifelse(latest.date.chicks - earliest.date.stage1 > min.fl, 1, 0), # did the nest reach the minimum fledging age
	fledged = ifelse(is.na(fledged), 0, fledged),
	focal.fail = ifelse(focal == 1 & active.last.day != 1 & fledged == 0, 1, 0), # was it a focal nest that failed?
	days.short.of.fledging = ifelse(max.stage >= 2 & fledged == 0,(latest.date.chicks - earliest.date.stage1) - min.fl, ""), # count number of days short of fledging the nest was observed; this is a liberal estimate, representing the largest possible difference between the minimum fledging age and [the number of days between the first observed incubation and last observed chicks]
	...
	hatch.by.stg1 = earliest.date.stage1 + inc.dur, # estimate of hatch date based on first observation of stage 1; assumes earliest.date.stage1 is first day of incubation, so this is likely later than true hatch date
...
focal_fail_dates = screened_nests %>% 
  filter(focal.fail==1) %>% 
  group_by(species, latest.date) %>%
	summarize(num.fails = length(nest)) %>% 
  select(date = latest.date, everything()) 
# join 	
...
focal_fail_dates_wide <- zdates %>%
  arrange(species, date) %>% 
  mutate(next.date = lead(date),
         next.md = paste(month(next.date), day(next.date), sep="-"),
         next.md = ifelse(next.md=="NA-NA", "", next.md)) %>% 
  select(species, num.fails, next.md) %>% 
  filter(num.fails>0) %>% 
  spread(next.md, num.fails)
...
make_nest_scoring <- function(){
  ...
  nests.sub <- screened_nests %>% 
    select(species, nest, focal, focal.fail, fledged, days.short.of.fledging, active.last.day, max.stage, num.spp, stg4.brood, first.check.fl.fa, inc.start.by.stg4, hatch.by.stg1, hatch.by.stg4)
...
hep.wide1=full_join(hep.wide, nests.sub) %>% 
  mutate(use.for.count = ifelse(focal.fail > 0 | fledged > 0 | max.stage > 0, 1, 0),
         ...
         hatch.by.stg1 = gsub(paste(seas, "-", sep = ""),"", hatch.by.stg1),
         hatch.by.stg4 = gsub(paste(seas, "-", sep = ""),"", hatch.by.stg4))

  hep.wide1.left <- hep.wide1 %>% 
    select(everything(), -focal, -focal.fail, -fledged, -days.short.of.fledging, -active.last.day, -max.stage, -num.spp, -stg4.brood, -first.check.fl.fa, -inc.start.by.stg4, -hatch.by.stg1, -hatch.by.stg4, -use.for.count)
  
  hep.wide1.right <- hep.wide1 %>% 
    select(focal, focal.fail, fledged, use.for.count, stg4.brood, days.short.of.fledging, active.last.day, max.stage, num.spp, first.check.fl.fa, inc.start.by.stg4, hatch.by.stg1, hatch.by.stg4)
...
## gets written to final xls
stg4_wide = hep.wide1 %>%
	group_by(species, stg4.brood) %>%
	summarize(num.nests = length(nest)) %>% 
  filter(stg4.brood>0) %>% 
  mutate(stg4.brood = paste("BRD", stg4.brood, sep = "")) %>% 
  spread(stg4.brood, num.nests) 
...
hep.wide1 %>% filter(num.spp>1) %>% nrow()
...
# 17 define export path and functions -------------
...
export.1.spp = function(zspp){
  ...
  hep.wide1spp <- hep.wide1 %>% 
    filter(species == zspp)
  hep.wide1spp2 <- select(hep.wide1spp, contains("-"))
    
  #hep.wide1spp2 <- hep.wide1spp[, 3:(ncol(hep.wide1spp)-11)]
  hep.wide1spp2noempty <- hep.wide1spp2[, colSums(hep.wide1spp2 != "") != 0]
  hep.wide1spp3 <- cbind(select(hep.wide1spp, species, nest), 
                         hep.wide1spp2noempty,
                         select(hep.wide1spp, focal, focal.fail, fledged, use.for.count, stg4.brood, days.short.of.fledging, inc.start.by.stg4, hatch.by.stg1, hatch.by.stg4))
...
export.multi.spp=function(){ 
    hep.wide1mspp <- hep.wide1 %>% 
      filter(num.spp > 1) %>% 
      select(everything(), -first.check.fl.fa, -active.last.day, -max.stage, -num.spp, inc.start.by.stg4, use.for.count) %>% 
